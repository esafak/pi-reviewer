      ...
      Type.Object(
        {
          ...
          line: Type.Integer({ description: "Line number in the file (not the diff position)." }),
...
export interface ReviewTool {
  tool: AgentTool<typeof reviewSchema, ReviewParams>;
 ...
 */
export function createReviewTool(): ReviewTool {
  let captured: ReviewResult | undefined;

  const tool: AgentTool<typeof reviewSchema, ReviewParams> = {
    name: "submit_review",
    label: "submit_review",
    description:
