...
describe("resolveCurrentModelId", () => {
  ...
  it("resolves from session model when no explicit model given", () => {
    ...
    expect(resolveCurrentModelId(undefined, models, session)).toBe("anthropic/claude-sonnet-4-6");
  });

  it("matches by bare model id", () => {
    expect(resolveCurrentModelId("gpt-4o", models, undefined)).toBe("openai/gpt-4o");
  });

  it("matches by provider/id string", () => {
    expect(resolveCurrentModelId("anthropic/claude-sonnet-4-6", models, undefined)).toBe(
      "anthropic/claude-sonnet-4-6",
    );
  });

  it("falls back to the raw model string when no registry match", () => {
    expect(resolveCurrentModelId("custom/unknown-model", models, undefined)).toBe(
  ...
  it("prefers explicit model over session model", () => {
    ...
    expect(resolveCurrentModelId("claude-sonnet-4-6", models, session)).toBe(
      "anthropic/claude-sonnet-4-6",
    );
  });
});
