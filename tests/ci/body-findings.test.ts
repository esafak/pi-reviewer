...
const finding = {
  findingId: bodyFindingId(["src/a.ts", 1, "RIGHT", "issue"].join("\0")),
  file: "src/a.ts",
  line: 1,
  side: "RIGHT" as const,
...
describe("body finding markers", () => {
  it("round trips multiple findings and assigns positive stable IDs", () => {
    const body = [
      ...
      encodeBodyFindingMarker({ ...finding, findingId: bodyFindingId("other") }),
    ].join("\n");
  ...
  it("collects only active authenticated review-body findings and partitions resolved history", () => {
    ...
    expect(result.resolvedFindings).toEqual([
      expect.objectContaining({
        historicalFindingId: `body:${bodyFindingId("resolved")}`,
        reviewId: 9,
        resolutionTargetSha: undefined,
      }),
    ]);
  });
  ...
  it("updates lifecycle metadata without changing visible review text", () => {
    ...
    expect(decodeBodyFindingMarkers(updated)[0]).toMatchObject({
  ...
  it("escapes comment terminators in metadata while preserving the finding text", () => {
    ...
    expect(
      updateBodyFindingMarker(encoded, finding.findingId, "RESOLVED", "head", "fixed"),
    ).toContain('"body":"keep -\\u002d> and -\\u002d!> visible"');
    expect(
      decodeBodyFindingMarkers(
        updateBodyFindingMarker(encoded, finding.findingId, "RESOLVED", "head", "fixed"),
      )[0],
    ).toMatchObject({ ...value, status: "RESOLVED", targetSha: "head", explanation: "fixed" });
  });
});
