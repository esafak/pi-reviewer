...
import {
  ...
  resolveAuthorizedLogins,
} from "../../src/ci/reply.js";
...
const event: Event = {
  ...
  commentId: 9,
  parentCommentId: 8,
  actor: { login: "human", association: "MEMBER", type: "User" },
};
const root: ReviewComment = {
  id: 8,
  body: "<!-- pi-reviewer:finding:v1 --> finding",
  path: "src/example.ts",
  line: 12,
  side: "RIGHT",
  user: { login: "reviewer[bot]", type: "Bot" },
};
const triggering: ReviewComment = {
  id: 9,
...
const thread: ReviewThread = {
  id: "thread-1",
  isResolved: false,
  comments: { nodes: [{ id: 8 }, { id: 9 }], pageInfo: { hasNextPage: false } },
};

function client(
  comments: ReviewComment[] = [root, triggering],
  current = pr,
  ...
  const updateReviewComment = vi.fn(
    async (_repo: string, _number: number, id: number, body: string) =>
      ({ id, body }) as ReviewComment,
  );
  const getCollaboratorPermission = vi.fn(
    async (_repo: string, login: string) => permissions[login] ?? "write",
  );
  return {
...
describe("review-comment reply action path", () => {
  it("discovers the oldest unprocessed direct reply per unresolved finding", () => {
    ...
    const handled: ReviewComment = {
      id: 11,
      body: replyMarker(9, 8, "thread-1"),
      in_reply_to_id: 8,
      user: { login: "reviewer[bot]", type: "Bot" },
    };
    const snapshot = {
      pullRequest: pr,
      comments: [root, first, second, handled],
      threads: [thread],
    };
    expect(
  ...
  it("does not let nested, unauthorized, resolved, or spoofed replies suppress recovery", () => {
    const secondRoot: ReviewComment = {
      id: 18,
      body: "<!-- pi-reviewer:finding:v1 --> second",
      user: { login: "reviewer[bot]", type: "Bot" },
    };
    const nested: ReviewComment = {
      id: 19,
      body: "nested",
      in_reply_to_id: 9,
      author_association: "MEMBER",
    ...
    const unauthorized: ReviewComment = {
      id: 20,
      body: "outsider",
      in_reply_to_id: 8,
    ...
    const resolvedThread: ReviewThread = {
      id: "thread-2",
      isResolved: true,
      comments: { nodes: [{ id: 18 }], pageInfo: { hasNextPage: false } },
    };
    const spoofed: ReviewComment = {
      id: 21,
      body: replyMarker(9, 8, "thread-1"),
      in_reply_to_id: 8,
      author_association: "MEMBER",
    ...
    const snapshot = {
      ...
      comments: [
        root,
        { ...triggering, user: { login: "other", type: "User" } },
        nested,
        unauthorized,
        secondRoot,
        spoofed,
      ],
      threads: [thread, resolvedThread],
    };
    expect(
      discoverPendingReplies(snapshot, { login: "reviewer[bot]" }, new Set(["human"])),
    ).toEqual([expect.objectContaining({ commentId: 21, parentCommentId: 8 })]);
  });
  ...
  it("recovers a canceled reply event through the push path", async () => {
    ...
    const generate = vi.fn(async () => ({
      action: "resolve",
      body: "The exception is intentional for this adoption PR.",
    }));
  ...
  it("refuses stale, fork, and snapshot-mismatched synchronize recovery", async () => {
    ...
    expect(
      await recoverSynchronizeReplies({
        ...
        identity: { login: "reviewer[bot]" },
        github,
        snapshot: { pullRequest: pr, comments: [], threads: [] },
        generate,
      }),
    ).toBe(0);
    expect(
      await recoverSynchronizeReplies({
        repo: "owner/repo",
        pullRequest: { ...pr, head: { ...pr.head, repo: { full_name: "fork/repo" } } },
        expectedHeadSha: "head",
        identity: { login: "reviewer[bot]" },
        github,
        snapshot: { pullRequest: pr, comments: [], threads: [] },
    ...
    expect(
      await recoverSynchronizeReplies({
        ...
        github,
        snapshot: {
          pullRequest: { ...pr, head: { ...pr.head, sha: "other" } },
          comments: [root, triggering],
          threads: [thread],
        },
        generate,
      }),
  ...
  it("continues recovery after one reply fails and does not invoke the model twice", async () => {
    ...
    const secondThread: ReviewThread = {
      id: "thread-2",
      isResolved: false,
      comments: { nodes: [{ id: 18 }, { id: 19 }], pageInfo: { hasNextPage: false } },
    };
    ...
    const snapshot = {
      pullRequest: pr,
      comments: [root, firstReply, secondRoot, secondReply],
      threads: [thread, secondThread],
    };
    expect(
      await recoverPendingReplies({
        repo: "owner/repo",
        pullRequest: pr,
        identity: { login: "reviewer[bot]" },
        github,
  ...
  it("does not discover bot-authored replies", () => {
    ...
    expect(
      discoverPendingReplies(
        { pullRequest: pr, comments: [root, botReply], threads: [thread] },
        { login: "reviewer[bot]" },
        new Set(["other-bot"]),
      ),
    ).toEqual([]);
  ...
  it("fails closed and logs when the permission lookup fails", async () => {
    const github = client();
    github.getCollaboratorPermission.mockRejectedValueOnce(
      new Error("403 Resource not accessible"),
    );
  ...
  it("resolves each candidate author once and excludes unknown permissions", async () => {
    const github = client();
    github.getCollaboratorPermission.mockImplementation(async (_repo: string, login: string) =>
      login === "admin" ? "admin" : undefined,
    );
    const authorized = await resolveAuthorizedLogins(github, "owner/repo", [
      "admin",
      "admin",
      "ghost",
    ]);
  ...
  it("decodes JSON-escaped Markdown line breaks in assistant replies", () => {
    expect(
      parseReplyAction({ action: "reply", body: "First paragraph\\n\\n- **second**" }),
    ).toEqual({
      action: "reply",
  ...
  it("posts one response rooted at the finding and remains idempotent", async () => {
    ...
    const generate = vi.fn(async () => ({
      action: "reply",
      body: "That is explained by the validation step.",
    }));
    ...
    expect(github.reply).toHaveBeenCalledTimes(1);
    expect(github.reply).toHaveBeenCalledWith(
      "owner/repo",
      42,
      8,
      `${replyMarker(9, 8, "thread-1")}\nThat is explained by the validation step.`,
    );
    expect(generate).toHaveBeenCalledTimes(1);
  });
  it("posts and resolves an explicit resolve action", async () => {
    ...
    const generate = vi.fn(async () => ({
      action: "resolve",
      body: "I am withdrawing this concern.",
    }));
    ...
    expect(github.updateReviewComment).toHaveBeenCalledWith(
      "owner/repo",
      42,
      10_002,
    ...
    expect(github.resolveThread).toHaveBeenCalledWith("thread-1");
    expect(github.resolveThread.mock.invocationCallOrder[0]).toBeLessThan(
      github.updateReviewComment.mock.invocationCallOrder[0],
    );
  });
  it("does not mark a reply resolved when thread resolution fails", async () => {
    ...
    expect(github.reply).toHaveBeenCalledWith(
      ...
      42,
      8,
      expect.stringContaining('"status":"STILL_OPEN"'),
    );
  ...
  it("does not leave a resolved marker when the head moves after posting", async () => {
    ...
    expect(github.reply).not.toHaveBeenCalledWith(
      "owner/repo",
      42,
      8,
  ...
  it("posts nothing for a malformed action", async () => {
    ...
    await handleReply({
      ...
      identity: { login: "reviewer[bot]" },
      github,
      generate: vi.fn(async () => "plain text"),
    });
    expect(github.reply).not.toHaveBeenCalled();
  ...
  it.each([
    [
      "human-rooted",
      { ...root, user: { login: "human" }, body: "human finding" },
      triggering,
      event,
      {},
    ],
    [
      "quoted-marker-root",
      { ...root, body: "Quoted <!-- pi-reviewer:finding:v1 --> finding" },
      triggering,
      event,
      {},
    ],
    [
      "variant-marker-root",
      { ...root, body: "<!-- pi-reviewer :finding:v1 --> finding" },
      triggering,
      event,
      {},
    ],
    [
      "bot-authored",
      root,
      { ...triggering, user: { login: "other-bot", type: "Bot" } },
      { ...event, actor: { login: "other-bot", association: "MEMBER", type: "Bot" } },
      {},
    ],
    [
      "no write permission",
      root,
      { ...triggering, user: { login: "outsider", type: "User" } },
      { ...event, actor: { login: "outsider", association: "CONTRIBUTOR", type: "User" } },
      { outsider: "read" },
    ],
    ["resolved", root, triggering, event, {}],
    ["stale head", root, triggering, { ...event, headSha: "old-head" }, {}],
  ])(
    "posts nothing for %s replies",
    async (name, parent, replyComment, replyEvent, permissions) => {
      const github = client(
        [parent as ReviewComment, replyComment as ReviewComment],
        pr,
        permissions as Record<string, string>,
      );
      if (name === "resolved")
        github.listThreads.mockResolvedValue([{ ...thread, isResolved: true }]);

      await handleReply({
        event: replyEvent as Event,
        repo: "owner/repo",
        pullRequest: pr,
        identity: { login: "reviewer[bot]" },
        github,
        generate: vi.fn(async () => ({ action: "reply", body: "answer" })),
      });
      expect(github.reply).not.toHaveBeenCalled();
    },
  );
  ...
  it("posts nothing when the authenticated duplicate marker already exists", async () => {
    const github = client([
      root,
      triggering,
      { id: 10, body: replyMarker(9, 8, "thread-1"), user: { login: "reviewer[bot]" } },
    ]);
    await handleReply({
      event,
      repo: "owner/repo",
  ...
  it.each([
    ...
    const github = client([
      root,
      triggering,
      { id: 10, body: marker, user: { login: "reviewer[bot]" } },
    ]);
    await handleReply({
      event,
      repo: "owner/repo",
  ...
  it("does not let a human-authored forged marker suppress a response", async () => {
    const github = client([
      root,
      triggering,
      { id: 10, body: replyMarker(9, 8, "thread-1"), user: { login: "human" } },
    ]);
    await handleReply({
      event,
      repo: "owner/repo",
