...
describe("batch markers", () => {
  it("round trips a versioned authenticated marker", () => {
    ...
    expect(decodeBatchMarker(encoded)).toEqual({
      version: 1,
      fromSha: "base",
      toSha: "head",
      kind: "synchronize",
  ...
  it("selects the whole PR for adoption and a later push range thereafter", () => {
    expect(selectBatchRange("base", "head")).toEqual({
      fromSha: "base",
      toSha: "head",
      fresh: true,
    });
    ...
    expect(selectBatchRange("base", "head", marker, () => true)).toEqual({
      fromSha: "old",
      toSha: "head",
      fresh: true,
    });
    expect(selectBatchRange("base", "head", marker, () => false)).toEqual({
      fromSha: "base",
      toSha: "head",
      fresh: true,
    });
    ...
    expect(
      isAuthorizedReviewCommand(
        normalizeEvent({
          ...
          comment: { body: "/pi-review", author_association: "NONE", user: { type: "User" } },
        }),
      ),
    ).toBe(false);
    expect(
      isAuthorizedReviewCommand(
        normalizeEvent({
          issue: { number: 1, pull_request: {} },
          comment: { body: "/pi-review", author_association: "MEMBER", user: { type: "Bot" } },
        }),
      ),
    ).toBe(false);
    const marker = encodeBatchMarker({
      fromSha: "a",
      toSha: "b",
      kind: "opened",
      actor: "attacker",
  ...
  it("handles coalesced pushes and idempotent reruns", () => {
    ...
    expect(selectBatchRange("base", "third", marker, () => true)).toMatchObject({
      fromSha: "first",
      toSha: "third",
      fresh: true,
    });
    expect(selectBatchRange("base", "first", marker, () => true)).toEqual({
      fromSha: "first",
      toSha: "first",
      fresh: false,
    });
    expect(
      normalizeEvent({
        ...
        pull_request: {
          number: 1,
          head: { sha: "third", repo: { full_name: "o/r" } },
  ...
  it("accepts authenticated markers from issue-comment-shaped sources", () => {
    ...
    expect(
      selectAuthenticatedBatchMarkers([{ id: 99, user: { login: "bot" }, body: marker }], "bot").at(
        -1,
      )?.toSha,
    ).toBe("new");
  });
...
describe("resolved finding history", () => {
  it("collects resolved inline conversations from every participant and body fallback history", () => {
    const fixture = JSON.parse(
      readFileSync(
        path.resolve(process.cwd(), "tests/fixtures/resolved-loki-pr-1604.json"),
        "utf8",
      ),
    ) as { pullRequest: number; resolvedReview: number; laterReview: number; provenance: string };
    expect(fixture).toMatchObject({
      pullRequest: 1604,
      resolvedReview: 5078197836,
      laterReview: 5083938684,
    });
    ...
    const bodyFinding = {
      ...
      body: "Loki values allow an unsafe configuration.",
      status: "RESOLVED" as const,
      targetSha: "resolved-sha",
      explanation: "human resolution",
    };
    const result = collectFindingHistory({
      ...
      reviews: [
        {
          id: 5078197836,
          body: `${encodeBatchMarker({ fromSha: "base", toSha: "old", kind: "opened", actor: "review-bot", reviewId: 5078197836 })}\n${encodeBodyFindingMarker(bodyFinding)}`,
      ...
      comments: [
        {
          id: 42,
          body: "<!-- pi-reviewer:finding:v1 -->\nLoki values allow an unsafe configuration.",
          user: { login: "review-bot" },
          path: "charts/loki/values.yaml",
          line: 42,
          side: "RIGHT",
          pull_request_review_id: 5078197836,
          created_at: "2026-01-01T00:00:00Z",
        },
        {
          id: 43,
          body: "I checked the deployment; this is safe.",
        ...
        {
          id: 44,
          body: `${replyMarker(9, 8, "thread-42")}\n<!-- pi-reviewer:status:v1 {"findingId":42,"targetSha":"resolved-sha","status":"RESOLVED"} --> Resolved`,
          user: { login: "review-bot" },
          in_reply_to_id: 42,
      ...
      threads: [
        {
          id: "thread-42",
          isResolved: true,
          comments: { nodes: [{ id: 42 }, { id: 43 }, { id: 44 }] },
        },
      ],
    });
    expect(result.activeFindings.map((f) => f.commentId)).toEqual([45]);
    expect(result.resolvedFindings.map((f) => f.historicalFindingId)).toEqual([
      "inline:42",
      "body:9001",
    ]);
  ...
  it("uses thread structure instead of quoted marker text to identify roots", () => {
    const result = collectFindingHistory({
      ...
      comments: [
        {
          id: 1,
          body: "<!-- pi-reviewer:finding:v1 --> issue mentions pi-reviewer:status:v1",
          user: { login: "review-bot" },
          path: "src/a.ts",
          line: 1,
          side: "RIGHT",
        ...
        {
          id: 2,
          body: "Quoted <!-- pi-reviewer:finding:v1 -->",
          user: { login: "review-bot" },
          in_reply_to_id: 1,
          path: "src/a.ts",
          line: 1,
  ...
  it("keeps identical text at different locations distinguishable", () => {
    ...
    const result = collectFindingHistory({
      ...
      comments,
      threads: [
        { id: "t1", isResolved: true, comments: { nodes: [{ id: 1 }] } },
        { id: "t2", isResolved: true, comments: { nodes: [{ id: 2 }] } },
      ],
    });
    expect(result.resolvedFindings.map((f) => f.historicalFindingId)).toEqual([
      "inline:1",
  ...
  it("keeps bot-authored lifecycle metadata authoritative across revisions", () => {
    const result = collectFindingHistory({
      ...
      comments: [
        ...
        {
          id: 2,
          body: 'Quoting the bot: <!-- pi-reviewer:status:v1 {"findingId":1,"targetSha":"fake","status":"RESOLVED"} --> looks resolved to me',
          user: { login: "maintainer" },
          in_reply_to_id: 1,
          created_at: "2026-01-01T00:01:00Z",
        },
        ...
        {
          id: 11,
          body: '<!-- pi-reviewer:status:v1 {"findingId":10,"targetSha":"prior-sha","status":"PARTIALLY_RESOLVED"} -->\nPartially addressed by prior-sha: improved',
          user: { login: "review-bot" },
          in_reply_to_id: 10,
        ...
        {
          id: 12,
          body: '<!-- pi-reviewer:status:v1 {"findingId":10,"targetSha":"current-sha","status":"RESOLVED"} -->\nResolved by current-sha: fully fixed',
          user: { login: "review-bot" },
          in_reply_to_id: 10,
      ...
      threads: [
        { id: "t1", isResolved: false, comments: { nodes: [{ id: 1 }, { id: 2 }] } },
        {
          id: "t2",
          isResolved: true,
          comments: { nodes: [{ id: 10 }, { id: 11 }, { id: 12 }, { id: 13 }] },
        },
      ],
    });
  ...
  it("keeps the latest bot status reply authoritative over later non-status bot replies", () => {
    const result = collectFindingHistory({
      ...
      comments: [
        ...
        {
          id: 2,
          body: '<!-- pi-reviewer:status:v1 {"findingId":1,"targetSha":"sha1","status":"PARTIALLY_RESOLVED"} -->\nPartially addressed by sha1: improved',
          user: { login: "review-bot" },
          in_reply_to_id: 1,
          created_at: "2026-01-01T00:01:00Z",
        },
        {
          id: 3,
          body: '<!-- pi-reviewer:reply:v1 {"version":1,"commentId":1,"parentId":1,"threadId":"t"} -->\nHere is the fix you asked for.',
          user: { login: "review-bot" },
          in_reply_to_id: 1,
          created_at: "2026-01-01T00:02:00Z",
        },
  ...
  it("decodes status metadata regardless of JSON key order", () => {
    const result = collectFindingHistory({
      ...
      comments: [
        ...
        {
          id: 2,
          body: '<!-- pi-reviewer:status:v1 {"status":"RESOLVED","findingId":1,"targetSha":"sha1"} -->\nResolved',
          user: { login: "review-bot" },
          in_reply_to_id: 1,
          created_at: "2026-01-01T00:01:00Z",
        },
    ...
    expect(result.resolvedFindings[0]).toMatchObject({
      resolutionTargetSha: "sha1",
  ...
  it("regresses the synthetic PR #1604 unchanged Loki review and permits only proven reraises", () => {
    const fixture = JSON.parse(
      ...
      finding: { file: string; line: number; side: "LEFT" | "RIGHT"; body: string };
      laterDiff: string;
    };
    const history = [
      { historicalFindingId: "inline:42", originalBody: fixture.finding.body, ...fixture.finding },
    ];
    const unchanged = parseAgentResponseWithStatus(
      JSON.stringify({
        summary: fixture.laterDiff,
        comments: [{ ...fixture.finding, severity: "WARN" }],
      }),
      "INFO",
      undefined,
      undefined,
    ...
    for (const reason of [
      ...
      const reraised = parseAgentResponseWithStatus(
          ...
          comments: [
            {
              ...
              re_raise_evidence: fixture.laterDiff,
            },
          ],
        }),
        "INFO",
        ...
        undefined,
        history,
        { commitId: "later-sha", batchMarker: "batch-marker" },
      );
  ...
  it("imports legacy active body markers and ignores malformed markers", () => {
    ...
    expect(result.activeFindings).toHaveLength(1);
    expect(result.activeFindings[0]).toMatchObject({
      historicalFindingId: "body:7",
      body: "legacy finding",
    });
  });
});
...
describe("event normalization", () => {
  it("normalizes PR and authorized comment events", () => {
    expect(
      normalizeEvent({
        ...
        pull_request: {
          number: 3,
          head: { sha: "h", repo: { full_name: "o/f" } },
          base: { repo: { full_name: "o/r" } },
        },
        sender: { login: "x", type: "User" },
      }),
    ).toMatchObject({ kind: "opened", pr: 3, headSha: "h", fork: true });
  ...
  it("rejects unsupported actions and checks both event SHAs", () => {
    expect(() => normalizeEvent({ action: "closed", pull_request: { number: 3 } })).toThrow(
      /Unsupported/,
    );
    const event = normalizeEvent({
      action: "synchronize",
      before: "old",
      after: "new",
      pull_request: { number: 3, head: { sha: "new" }, base: {} },
    });
  ...
  it("normalizes review-comment replies without treating them as reviews", () => {
    ...
    expect(reply).toMatchObject({
      ...
      parentCommentId: 8,
      fork: false,
      actor: { login: "human", association: "CONTRIBUTOR", type: "User" },
    });
  });
  it("normalizes a top-level review comment as an unusable reply with no parent", () => {
    expect(
      normalizeEvent({
        ...
        comment: { id: 9, body: "new finding", user: { login: "human", type: "User" } },
      }),
    ).toMatchObject({ kind: "reply", parentCommentId: undefined });
  });
  it("requires a root finding and authenticates reply marker ownership at selection time", () => {
    ...
    expect(
      isPiReviewerRootComment({ body: "Quoted <!-- pi-reviewer:finding:v1 --> finding" }),
    ).toBe(false);
    expect(isPiReviewerRootComment({ body: "<!--pi-reviewer:finding:v1--> finding" })).toBe(false);
    expect(isPiReviewerRootComment({ body: "<!-- pi-reviewer :finding:v1 --> finding" })).toBe(
      false,
    );
    expect(decodeReplyMarker(replyMarker(9, 8, "thread-1"))).toEqual({
      version: 1,
      commentId: 9,
      parentId: 8,
      threadId: "thread-1",
