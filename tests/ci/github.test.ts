...
describe("GitHubClient", () => {
  ...
  it("uses the authenticated custom App identity", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(response({ login: "review-app[bot]", id: 42, type: "Bot" }));
    vi.stubGlobal("fetch", fetchMock);
    await expect(new GitHubClient("token").getUser()).resolves.toMatchObject({
  ...
  it("uses GraphQL viewer identity for installation tokens", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        response({ message: "Resource not accessible by integration" }, false, 403),
      )
      .mockResolvedValueOnce(
        response({ data: { viewer: { login: "review-app[bot]", id: "42", __typename: "Bot" } } }),
      );
    vi.stubGlobal("fetch", fetchMock);
    await expect(new GitHubClient("token").getUser()).resolves.toMatchObject({
      login: "review-app[bot]",
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ authorization: "Bearer token" }),
        body: JSON.stringify({ query: githubGraphqlDocuments.viewer, variables: {} }),
      }),
    );
  });
  ...
  it("creates a reaction on a specific review comment", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.github.com/repos/owner/repo/pulls/42/comments/99/reactions",
      expect.objectContaining({ method: "POST", body: JSON.stringify({ content: "eyes" }) }),
    );
  });
  it("resolves a review thread through GraphQL", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          query: githubGraphqlDocuments.resolveThread,
  ...
  it("updates a review comment", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.github.com/repos/owner/repo/pulls/comments/9",
  ...
  it("fetches an individual review body", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(new GitHubClient("token").getReview("owner/repo", 1, 42)).resolves.toMatchObject({
      body: "fresh body",
    });
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.github.com/repos/owner/repo/pulls/1/reviews/42",
      expect.anything(),
    );
  });
  it("fetches a collaborator's repository permission", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(
      new GitHubClient("token").getCollaboratorPermission("owner/repo", "human"),
    ).resolves.toBe("admin");
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.github.com/repos/owner/repo/collaborators/human/permission",
      expect.anything(),
    );
  });
  it("follows GraphQL thread cursors", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        response({
          data: {
            node: { comments: { nodes: [{ id: "102" }], pageInfo: { hasNextPage: false } } },
          },
        }),
      )
      .mockResolvedValueOnce(
    ...
    expect(
      fetchMock.mock.calls.every(
        ([url, init]) =>
          ...
          !JSON.parse((init as RequestInit).body as string).query.match(/\bdatabaseId\b|\bside\b/),
      ),
    ).toBe(true);
  });
                ...
                nodes: [
                  {
                    id: "t1",
                    isResolved: false,
                    comments: { nodes: [{ id: "2778522194" }], pageInfo: { hasNextPage: false } },
                  },
                ],
                pageInfo: { hasNextPage: false },
              },
