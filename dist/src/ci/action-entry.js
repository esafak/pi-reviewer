...
function ancestor(from, to, cwd = process.cwd()) { try {
...
if (event.kind === "manual" && event.command !== "/pi-review" && process.env.GITHUB_EVENT_NAME === "issue_comment") {
    process.exit(0);
}
...
try {
    mergeBase = execFileSync("git", ["merge-base", pr.base.sha, head], { cwd: process.cwd(), encoding: "utf8" }).trim() || mergeBase;
}
catch {
...
if (!isEventRangeConsistent(event, latest?.toSha ?? mergeBase, head))
    console.warn(`[pi-reviewer] event SHAs differ from authenticated PR state; using authenticated marker range`);
